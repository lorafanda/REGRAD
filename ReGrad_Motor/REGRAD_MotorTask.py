...
# Define Paths: Do not change is same folder structure is being used
folder_path = os.path.dirname(os.path.abspath(__file__))
SoundFile= os.path.join(folder_path,'Go.wav') # TODO: Change Tone
SoundFile_End= os.path.join(folder_path,'Yay_sound.wav') # TODO: Change Tone
Respath= 'Results'
...
def onetrial(mywin,R,Center,Color,FileName,TrialNumber, TrialName):
    ...
    # Set text
    ParticipantSide=visual.TextStim(win=mywin,text="",color=[1,1,1],pos=[-300,0],anchorHoriz='center',height=30)
    IntReultsText=visual.TextStim(win=mywin,text="",color=[1,1,1],pos=[0,-200],anchorHoriz='center',height=20)
    IntReultsText.setText(TrialName + ': ' + str(TrialNumber+1)+'/'+str(60)+'\nPress Space to play "beep"')
    IntReultsText.draw() 
    ...
    while True:
        ...
        if Exit:
            break
        if len(event.getKeys(keyList=['num_0','0']))>0:
            Correctness = -1
            break        
        if len(event.getKeys(keyList=['space']))>0:
            Correctness = 1
...
def createTargetFlankerPositions_randomized():
    # Returns positions (60 times 2x3 position array) and names (60 times 'TxFx' labels)
    # Visualizing Positions:        
    # 1 = Target, 2 = Flanker       __box______
    #                               |0   1   0|
    #     [[0,1,0],[2,0,0]]    =    |2   0   0|
    #                               ```````````
    # 5 repetitions for each position, total 12 positions = 60 trials per eye condition
    IndexPositions=[]
    CodeNames = []

    # Depth 0:
    T1F1 = np.array([[1,0,0],[2,0,0]])
    ...
    position_list=[T1F1, F1T1, T3F3, F3T3, T1F2, F2T1, T3F2, F2T3, T1F3, F1T3, T3F1, F3T1]
    name_list=['T1F1', 'F1T1', 'T3F3', 'F3T3', 'T1F2', 'F2T1', 'T3F2', 'F2T3', 'T1F3', 'F1T3', 'T3F1', 'F3T1']
    [[IndexPositions.append(i) for i in position_list] for i in range(5)]
...
while True:
    ...
    if InitialData==['', 'ND','', 'M', 'R', 'REV','T1',1]: # Cancel if press
        Exp=False
        break
    else:
        ...
        FileName=SbjNumber+'_'+FirstCondition+'_'+Timepoint+'_'+Study+'.txt'
...
# Run experiment if Exp is true
if Exp:
        mywin=visual.Window([800,500], pos=[0,0], monitor="default",waitBlanking=True,units="pix",color='black',fullscr=False,allowGUI=True)
        ...
        if FirstCondition=='amblyopic non-dominant eye':
            IntroFile= os.path.join(folder_path, 'instructionscreen_amblyopic.png')
        elif FirstCondition == "both eyes":
            IntroFile= os.path.join(folder_path, folder_path,'instructionscreen_botheyes.png')
        else:
            IntroFile= os.path.join(folder_path, folder_path,'instructionscreen_nonamblyopic.png')
        ...
        for i,condition in enumerate(Conditions):
            if Exit:
                break
            ReGraDTaskIntro=visual.SimpleImageStim(win=mywin,image=os.path.join(folder_path, 'instructionscreen.png'))
            ...
            for trial_num in range(len(TrialName)):
                ...
                if i==60: # if all initial trials have been tried
                    Sound2 = sound.Sound(SoundFile_End,volume=Volume) #SoundFile_End # SYNTAX ERROR HERE
                    Sound2.play()
                    # mywin.flip()
